...
const TransitMarkerIcon = ({ color, index, isPending = false }: TransitMarkerProps) => {
  return (
    <svg
      width="41"
      height="49"
      ...
      className={`${color} ${isPending ? PENDING_COLOR.hover : "hover:text-gray-900"} group`}
    >
      {/* 배경 모양 */}
      ...
      <text
        ...
        fontSize="16"
        fontWeight="bold"
        className={`${isPending ? "text-gray-300 group-hover:text-gray-700" : "text-gray-900"}`}
      >
        {index}
      </text>
    </svg>
