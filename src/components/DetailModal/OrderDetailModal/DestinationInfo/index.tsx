import {
  ...
  ORDER_DETAIL_DELIVERY_INFO_LABEL,
...
interface DestinationInfoProps {
  info: NonNullable<OrderDetail["destinationInfo"]>;
}
  ...
  return (
    <div className="flex flex-wrap gap-[18px] gap-x-4 gap-y-[18px] rounded-lg bg-white px-4 py-5">
      ...
      {ORDER_DETAIL_DELIVERY_INFO_KEYS.map(({ key, line }) => {
        return <InfoItem key={key} label={ORDER_DETAIL_DELIVERY_INFO_LABEL[key]} value={info[key]} line={line} />;
      })}
    </div>
  );
};
